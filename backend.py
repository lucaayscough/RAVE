...
# A simple way to represent an op used in RaveBackend
...
@final
class RaveBackend(BackendDetails):
    """
    ...
    lowered_sin_module = to_backend(
        "RaveBackend", edgeir_m, compile_specs
    )
        ...
        for node in edge_program.graph.nodes:
            if node.op == "call_function":
                ...
                if node.target in match_ops:
                    ...
                    raise RuntimeError(
                        f"{node.op} {node.target.__name__} is not supported in backend RaveBackend"
                    )
                ...
                raise RuntimeError(
                    f"{node.op} is not supported in backend RaveBackend"
                )
